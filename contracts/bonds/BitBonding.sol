...
contract BitBonding {
    ...
    IERC20 public immutable BTR; // token paid for principal
    ...
    bool public immutable feeInBTR;
    ...
    struct MinAmountsLiq {
        uint256 minBTRToSwap;
        uint256 minBTRToAdd;
        uint256 mincoreBTCToAdd;
    }
    ...
    constructor(
        ...
        BTR = IERC20(ITreasury(_treasury).BTR());
        ...
        defaultToken = 0x349507EF6bc6311d1fF6D633BfbBEdf69d7eB28A;
    ...
    function initializeBond(
        ...
        if (_bondType == BondType.CBTCTORBTC) {
            require(address(principalToken) == defaultToken, "Principal must be coreBTC");
            principalToken.approve(rBTC, type(uint256).max);
        } else if (_bondType == BondType.CBTCTOLP) {
            require(address(principalToken) ==defaultToken, "Principal must be coreBTC");
            LP = treasury.LP();
            principalToken.approve(address(uniswapV2Router), type(uint256).max);
            BTR.approve(address(uniswapV2Router), type(uint256).max);
        }
    ...
    function withdrawStuckToken(
        ...
        require(_token != address(0), "_token address cannot be 0");
        require(_token != address(BTR), "Can not withdraw BTR");
    ...
    function deposit(
        ...
        if (!feeInBTR) _amount = _amount.sub(fee);

        require(payout >= 10 ** BTR.decimals() / 100, "Bond too small"); // must be > 0.01 payout token ( underflow protection )
        ...
        } else if (bondType == BondType.CBTCTORBTC) {
            principalToken.safeTransferFrom(msg.sender, address(this), _amount);
            IrBTC(rBTC).deposit(address(principalToken), address(treasury), _amount);
        } else {
            principalToken.safeTransferFrom(msg.sender, address(this), _amount);

            uint256 btrBefore = BTR.balanceOf(address(this));
            swapBTCForTokens(_amount / 2,_minAmounts.minBTRToSwap);
            addLiquidity(
                BTR.balanceOf(address(this)) - btrBefore,
                principalToken.balanceOf(address(this)),
                _minAmounts.minBTRToAdd,
                _minAmounts.mincoreBTCToAdd
            );
        }
    ...
    function redeem(address _depositor) external returns (uint256) {
        ...
        if (percentVested >= 10000) {
            ...
            BTR.safeTransfer(_depositor, info.payout);
            return info.payout;
        } else {
            // if unfinished
            ...
            BTR.safeTransfer(_depositor, payout);
            return payout;
        }
    }
    ...
    /// @dev INTERNAL function to swap `btcAmount` for BTR
    function swapBTCForTokens(uint256 btcAmount, uint256 minBTR) internal {
    ...
    /// @dev INTERNAL function to add `tokenAmount` and `btcAmount` to LP
    function addLiquidity(uint256 tokenAmount, uint256 btcAmount, uint256 minBTR, uint256 mincoreBTC) internal {
        // add the liquidity
    ...
    function maxPayout() public view returns (uint256) {
        return BTR.totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        debtRatio_ = FixedPoint
            .fraction(
                currentDebt().mul(10 ** BTR.decimals()),
                BTR.totalSupply()
            )
            .decode112with18()
            .div(1e18);
